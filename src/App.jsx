...
export default function App() {
  ...
  const [eF,  setEF]  = useState({ name:"",sku:"",qty:"",buy:"",sell:"",min:"",img:"",cat:"" });
        ...
        return (
          <div key={s.id} style={{ ...card,padding:"16px 16px" }}>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:7 }}>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:8,marginTop:2 }}><span style={{ fontSize:"1rem",flexShrink:0 }}>📋</span><span style={{ fontSize:"0.80rem",color:"#8888a0" }}>{totalOrders} общо заявки</span></div>
            </div>
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",minHeight:"100vh",background:"#0f0f14",color:"#e8e8f0",fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <Modal open={!!mEdit} onClose={()=>setMEdit(null)} title={`✏️ ${editProd?.name||""}`}
        ...
        <FG label="Категория"><input style={IS} placeholder="напр. Бонбони, Вода, Чипс..." value={eF.cat} onChange={e=>setEF(f=>({...f,cat:e.target.value}))} /></FG>
